...
const cursor = document.getElementById("ink-cursor")
  ...
  , width = 26
  , idleTimeout = 150;
let timeoutID, hoverButton, hoverTL, lastFrame = 0, mousePosition = {
  x: 0,
  y: 0
}, dots = [], idle = !1;
class HoverButton {
...
class Dot {
  constructor(e = 0) {
    this.index = e,
      ...
      this.limit = .75 * width * this.scale,
...
const onMouseMove = e => {
  ...
  , inkTablet = window.matchMedia("(max-width: 767px)")
  , inkMobile = window.matchMedia("(max-width: 479px)")
  , inkMediaObserver = () => {
    inkMobile.matches || inkTablet.matches || cursorInit || (cursorInit = !0,
      init())
  }
  ;
inkMediaObserver(),
